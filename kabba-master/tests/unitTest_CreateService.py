...
class ll_ATS(unittest.TestCase):
    ...
    def test_ll(self):
        ...
        # find products
        elem = driver.find_element(By.XPATH, '//*[@id="navbarNavAltMarkup"]/div/a[3]').click()

        time.sleep(3)

        elem = driver.find_element(By.XPATH, '/html/body/div/a').click()

        time.sleep(5)  # pause to allow screen to change
        try:

            elem = driver.find_element(By.XPATH, '/html/body/div/div/div/form/button')
            print("Test passed - products can be added")
            assert True

        except NoSuchElementException:
            self.fail("Service created")
